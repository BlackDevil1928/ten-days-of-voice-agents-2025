...
# LiveKit agent framework imports
...
# Plugins: use the imports compatible with your installed livekit package
from livekit.plugins import silero, google, murf, deepgram, noise_cancellation

# >>> FIX: import the turn detector class available in your environment
...
# Create a .env.local or .env in the same folder if required.
...
def get_db_path():
    # ensure path resolves even if __file__ is run from another cwd
...
def seed_database():
    ...
    if cur.fetchone()[0] == 0:
        ...
        conn.commit()
        print(f"✅ SQLite DB seeded at {path}")
...
async def resolve_fraud_case(
    ...
    try:
        ...
        logger.info(f"✅ CASE UPDATED: {case.userName} -> {status}")
...
class FraudAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Alex', a Fraud Detection Specialist at VIPU national Bank.
...
def prewarm(proc: JobProcess):
    # load VAD into process userdata so entrypoint can reuse it
    proc.userdata["vad"] = silero.VAD.load()


async def entrypoint(ctx: JobContext):
    ctx.log_context_fields = {"room": ctx.room.name}
    logger.info("🚀 STARTING FRAUD ALERT SESSION (SQLite)")

    userdata = Userdata()

    # >>> USE the MultilingualModel from the multilingual module (this matches your livekit install)
    session = AgentSession(
        ...
        turn_detection=MultilingualModel(),   # <-- CORRECT: MultilingualModel
